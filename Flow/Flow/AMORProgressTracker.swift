...
final class AMORProgressTracker {
    ...
    func computeStreakStats(practices: [PracticeStreak]) -> StreakStats {
    ...
    private func computeSessionsPerDay(sessions: [DailySession], weekStart: Date) -> [DayCount] {
        let calendar = Calendar.current
    ...
    private func computeTrend(currentSessions: Int, allSessions: [DailySession], weekStart: Date) -> TrendDirection {
        let calendar = Calendar.current
