...
enum AMORNudgeEngine {
    ...
    static func generateCronNudges(cronJobs: [CronJobHealth]) -> [AMORNudgeDescriptor] {
        var nudges: [AMORNudgeDescriptor] = []
    ...
    static func generateScheduledNudges(referenceDate: Date = .now) -> [AMORNudgeDescriptor] {
        ...
        let settings = AMORSettingsManager.shared
        let timeString = settings.briefingReminderTime
    ...
    static func isNotificationsEnabled() -> Bool {
        AMORSettingsManager.shared.notificationsEnabled
    }
