 ...
 *
 * Pure-function enum. No state. Reads PracticeStreak @Model objects.
 * Generates actionable streak insights, break risk assessments, and
 * personalized recovery suggestions.
 *
 * Architecture: Foundation-only. Type-checks with swiftc -typecheck -sdk macosx
 * (does NOT import SwiftUI). The @Model PracticeStreak is passed as parameter.
 */
...
enum AMORStreakIntelligence {
    ...
    static func assessRisk(practice: PracticeStreak, referenceDate: Date = .now) -> StreakRiskLevel {
    ...
    static func generateInsight(practice: PracticeStreak, referenceDate: Date = .now) -> StreakInsight {
    ...
    static func generateSummary(practices: [PracticeStreak], referenceDate: Date = .now) -> StreakSummary {
        let insights = practices.map { generateInsight(practice: $0, referenceDate: referenceDate) }
            .sorted { $0.priority < $1.priority }

        let activeCount = practices.filter { $0.isActive }.count
        ...
        let longest = practices.map { $0.currentStreak }.max() ?? 0
        let totalCompletions = practices.reduce(0) { $0 + $1.totalCompletions }

        // Health score: weighted by active streaks, penalties for at-risk and broken
        let total = max(practices.count, 1)
        let safeScore: Double = Double(safeCount) / Double(total) * 60.0
        ...
        let riskPenalty: Double = Double(atRiskCount + brokenCount) / Double(total) * 10.0
        ...
        } else if activeCount == practices.count && !practices.isEmpty {
            headline = "All practices on track"
    ...
    static func detectMilestones(practices: [PracticeStreak]) -> [String] {
    ...
    static func recoveryActions(for practices: [PracticeStreak], referenceDate: Date = .now) -> [String] {
        let insights = practices.map { generateInsight(practice: $0, referenceDate: referenceDate) }
