...
enum AMORBriefingEngine {
    ...
    private static func personalizedGreeting(phase: BriefingPhase, practices: [AMORPracticeSnapshot], date: Date) -> String {
        let cal = Calendar.current
    ...
    private static func buildAlerts(phase: BriefingPhase, today: TodaySnapshot, yesterday: YesterdayRecap, cronJobs: [AMORCronJobSnapshot]) -> [BriefingAlert] {
    ...
    private static func buildActions(phase: BriefingPhase, today: TodaySnapshot, practices: [AMORPracticeSnapshot]) -> [BriefingAction] {
