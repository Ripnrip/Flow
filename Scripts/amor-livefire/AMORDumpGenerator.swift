...
final class AMORDumpGenerator {
    ...
    func generateDailyDump(
        ...
        if !practices.isEmpty {
            ...
            for practice in practices {
                // v5.2.0: identity by practiceName — the ledgers' natural key (snapshots carry no UUID)
                let status = todayPractices.contains(where: { $0.practiceName == practice.practiceName }) ? "✅" : "⬜"
    ...
    func generateJSONExport(sessions: [AMORSessionSnapshot]) -> String? {
    ...
    func generateCSVExport(sessions: [AMORSessionSnapshot]) -> String {
